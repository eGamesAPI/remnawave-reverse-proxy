...
:::note
Не забываем добавить этот сервер в сеть NetBird, как это сделать, можете посмотреть [тут](../configuration/netbird).
:::
...
4. Вставляем следующее содержимое:
    ...
    server {
        ...
        ssl_certificate "/etc/letsencrypt/live/yourdomain.com/fullchain.pem"; # Указываем путь до сертификата
        ssl_certificate_key "/etc/letsencrypt/live/yourdomain.com/privkey.pem"; # Указываем путь до приватного ключа
        ssl_trusted_certificate "/etc/letsencrypt/live/yourdomain.com/fullchain.pem"; # Указываем путь до сертификата
...
8. Создаем файл службы vmagent:
    ...
    ExecStart=/opt/monitoring/vmagent/vmagent \
          ...
          -remoteWrite.url=http://IP:8428/api/v1/write # Тут указываем адрес Netbird (в данном случае сервера Grafana)
    Restart=always
    RestartSec=5
...
3. Попадаем в главное меню Grafana
    ...
    ![Grafana main menu](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/en/grafana/grafana_main_menu.png)
...
6. В секции Connection в поле `URL` вводим `http://vmsingle:8428`
    ...
    ![Grafana data source](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/en/grafana/grafana_connection.png)
...
- [cAdvisor Dashboard](https://raw.githubusercontent.com/pluralplay/awesome-docs/main/grafana/dashboards/cadvisor.json) от [Grafana](https://github.com/grafana)
...
4. Где victoriametrics выбираем наш prometheus
    ...
    ![Grafana Dashboard](https://raw.githubusercontent.com/eGamesAPI/remnawave-reverse-proxy/refs/heads/main/docs/src/assets/en/grafana/grafana_dashboard.png)
