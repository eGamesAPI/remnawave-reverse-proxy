...
declare -A LANG=(
    ...
    [INSTALL_PANEL_NODE]="Установить панель и ноду на один сервер (не рекомендуется)"
    ...
    [WARNING_PANEL_NODE_TOGETHER]="ВНИМАНИЕ: Установка панели и ноды на одном сервере не рекомендуется для продакшн среды!\nРекомендуется устанавливать панель и ноды на разных серверах для лучшей производительности и безопасности."
...
start_panel_node() {
    local panel_dir="${LANG[PANEL_DIR]}"
    local node_dir="${LANG[NODE_DIR]}"
    
    # Start panel if exists
    if [ -d "$panel_dir" ]; then
        cd "$panel_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $panel_dir${COLOR_RESET}"; exit 1; }
        
        if docker ps -q --filter "ancestor=remnawave/backend:latest" | grep -q . || docker ps -q --filter "ancestor=remnawave/backend:2" | grep -q .; then
            echo -e "${COLOR_GREEN}Панель уже запущена${COLOR_RESET}"
        else
            echo -e "${COLOR_YELLOW}Запуск панели...${COLOR_RESET}"
            sleep 1
            docker compose up -d > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            echo -e "${COLOR_GREEN}Панель запущена${COLOR_RESET}"
        fi
    fi
    
    # Start node if exists
    if [ -d "$node_dir" ]; then
        cd "$node_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $node_dir${COLOR_RESET}"; exit 1; }
        
        if docker ps -q --filter "ancestor=remnawave/node:latest" | grep -q .; then
            echo -e "${COLOR_GREEN}Нода уже запущена${COLOR_RESET}"
        else
            echo -e "${COLOR_YELLOW}Запуск ноды...${COLOR_RESET}"
            sleep 1
            docker compose up -d > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            echo -e "${COLOR_GREEN}Нода запущена${COLOR_RESET}"
        fi
    fi
...
stop_panel_node() {
    ...
    # Stop panel if exists
    if [ -d "$panel_dir" ]; then
        cd "$panel_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $panel_dir${COLOR_RESET}"; exit 1; }
        
        if docker ps -q --filter "ancestor=remnawave/backend:latest" | grep -q . || docker ps -q --filter "ancestor=remnawave/backend:2" | grep -q .; then
            echo -e "${COLOR_YELLOW}Остановка панели...${COLOR_RESET}"
            docker compose down > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            echo -e "${COLOR_GREEN}Панель остановлена${COLOR_RESET}"
        else
            echo -e "${COLOR_GREEN}Панель уже остановлена${COLOR_RESET}"
        fi
    fi
    ...
    if [ -d "$node_dir" ]; then
        ...
        if docker ps -q --filter "ancestor=remnawave/node:latest" | grep -q .; then
            echo -e "${COLOR_YELLOW}Остановка ноды...${COLOR_RESET}"
            docker compose down > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            echo -e "${COLOR_GREEN}Нода остановлена${COLOR_RESET}"
        else
            echo -e "${COLOR_GREEN}Нода уже остановлена${COLOR_RESET}"
        fi
    fi
...
update_panel_node() {
    ...
    if [ -d "$panel_dir" ]; then
        cd "$panel_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $panel_dir${COLOR_RESET}"; exit 1; }
        echo -e "${COLOR_YELLOW}Обновление панели...${COLOR_RESET}"
        sleep 1
        ...
        if [ "$before" != "$after" ] || echo "$pull_output" | grep -q "Pull complete"; then
            echo -e ""
            echo -e "${COLOR_YELLOW}Обнаружены новые образы, перезапускаем контейнеры...${COLOR_RESET}"
            docker compose down > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            sleep 5
            docker compose up -d > /dev/null 2>&1 &
            ...
            echo -e "${COLOR_GREEN}Панель успешно обновлена${COLOR_RESET}"
            updated=true
        else
            echo -e "${COLOR_YELLOW}Нет доступных обновлений для панели${COLOR_RESET}"
        fi
    fi
    ...
    if [ -d "$node_dir" ]; then
        cd "$node_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $node_dir${COLOR_RESET}"; exit 1; }
        echo -e "${COLOR_YELLOW}Обновление ноды...${COLOR_RESET}"
        sleep 1
        ...
        if [ "$before" != "$after" ] || echo "$pull_output" | grep -q "Pull complete"; then
            ...
            sleep 1
            docker image prune -f > /dev/null 2>&1
            echo -e "${COLOR_GREEN}Нода успешно обновлена${COLOR_RESET}"
            updated=true
        else
            echo -e "${COLOR_YELLOW}Нет доступных обновлений для ноды${COLOR_RESET}"
        fi
    fi
    ...
    if [ "$updated" = false ]; then
        echo -e "${COLOR_YELLOW}Нет доступных обновлений для панели/ноды${COLOR_RESET}"
    fi
}
...
remove_script() {
    ...
    case $SUB_OPTION in
        ...
        2)
            ...
            if [ -d "${LANG[PANEL_DIR]}" ]; then
                cd "${LANG[PANEL_DIR]}" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} ${LANG[PANEL_DIR]}${COLOR_RESET}"; exit 1; }
                docker compose down -v --rmi all --remove-orphans > /dev/null 2>&1 &
                spinner $! "${LANG[WAITING]}"
                rm -rf "${LANG[PANEL_DIR]}" 2>/dev/null
            fi
            
            # Remove node if exists
            if [ -d "${LANG[NODE_DIR]}" ]; then
                cd "${LANG[NODE_DIR]}" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} ${LANG[NODE_DIR]}${COLOR_RESET}"; exit 1; }
                docker compose down -v --rmi all --remove-orphans > /dev/null 2>&1 &
                spinner $! "${LANG[WAITING]}"
                rm -rf "${LANG[NODE_DIR]}" 2>/dev/null
            fi
...
manage_install() {
    ...
    case $INSTALL_OPTION in
        1)
            ...
            echo -e "${COLOR_RED}${LANG[WARNING_PANEL_NODE_TOGETHER]}${COLOR_RESET}"
            echo -e ""
...
view_logs() {
    ...
    echo -e "${COLOR_GREEN}Выберите логи для просмотра:${COLOR_RESET}"
    echo -e ""
    echo -e "${COLOR_YELLOW}1. Логи панели${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}2. Логи ноды${COLOR_RESET}"
    echo -e ""
    echo -e "${COLOR_YELLOW}0. Выход${COLOR_RESET}"
    echo -e ""
    reading "Выберите действие (0-2):" LOG_OPTION
    
    case $LOG_OPTION in
        1)
            if [ ! -d "$panel_dir" ]; then
                echo -e "${COLOR_RED}Панель не установлена${COLOR_RESET}"
                exit 1
            fi
            cd "$panel_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $panel_dir${COLOR_RESET}"; exit 1; }
            
            if ! docker ps -q --filter "ancestor=remnawave/backend:latest" | grep -q . && ! docker ps -q --filter "ancestor=remnawave/backend:2" | grep -q .; then
                echo -e "${COLOR_RED}Контейнер панели не запущен${COLOR_RESET}"
                exit 1
            fi
            
            echo -e "${COLOR_YELLOW}Просмотр логов панели${COLOR_RESET}"
            docker compose logs -f -t
            ;;
        2)
            if [ ! -d "$node_dir" ]; then
                echo -e "${COLOR_RED}Нода не установлена${COLOR_RESET}"
                exit 1
            fi
            cd "$node_dir" || { echo -e "${COLOR_RED}${LANG[CHANGE_DIR_FAILED]} $node_dir${COLOR_RESET}"; exit 1; }
            
            if ! docker ps -q --filter "ancestor=remnawave/node:latest" | grep -q .; then
                echo -e "${COLOR_RED}Контейнер ноды не запущен${COLOR_RESET}"
                exit 1
            fi
            
            echo -e "${COLOR_YELLOW}Просмотр логов ноды${COLOR_RESET}"
            docker compose logs -f -t
            ;;
        0)
            ...
            ;;
        *)
            echo -e "${COLOR_YELLOW}Неверный выбор${COLOR_RESET}"
            sleep 2
            view_logs
            ;;
    esac
...
choose_reinstall_type() {
    ...
    case $REINSTALL_OPTION in
        1|2|3)
            ...
            read confirm
            if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
                reinstall_remnawave "$REINSTALL_OPTION"
                ...
                case $REINSTALL_OPTION in
                    1) 
                        ...
                        echo -e "${COLOR_RED}${LANG[WARNING_PANEL_NODE_TOGETHER]}${COLOR_RESET}"
                        echo -e ""
                        reading "${LANG[CONFIRM_PROMPT]}" confirm2
...
reinstall_remnawave() {
    local reinstall_type="$1"
    
    # Remove panel if selected or if installing panel+node
    if [ "$reinstall_type" = "1" ] || [ "$reinstall_type" = "2" ]; then
        if [ -d "${LANG[PANEL_DIR]}" ]; then
            cd "${LANG[PANEL_DIR]}" || return
            docker compose down -v --rmi all --remove-orphans > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            rm -rf "${LANG[PANEL_DIR]}" 2>/dev/null
        fi
    ...
    # Remove node if selected or if installing panel+node
    if [ "$reinstall_type" = "1" ] || [ "$reinstall_type" = "3" ]; then
        if [ -d "${LANG[NODE_DIR]}" ]; then
            cd "${LANG[NODE_DIR]}" || return
            docker compose down -v --rmi all --remove-orphans > /dev/null 2>&1 &
            spinner $! "${LANG[WAITING]}"
            rm -rf "${LANG[NODE_DIR]}" 2>/dev/null
        fi
    ...
    case $EXTENSION_OPTION in
        ...
        2)
            if [ -f ~/backup-restore.sh ]; then
                ...
                curl -o ~/backup-restore.sh https://raw.githubusercontent.com/distillium/remnawave-backup-restore/main/backup-restore.sh && chmod +x ~/backup-restore.sh && bash ~/backup-restore.sh
            fi
            log_clear
            manage_extensions
            ;;
...
EOL
    ...
    # Install node in separate directory
    echo -e "${COLOR_YELLOW}Установка ноды в отдельной директории...${COLOR_RESET}"
    sleep 1
    mkdir -p "${LANG[NODE_DIR]}" && cd "${LANG[NODE_DIR]}"
...
EOL
    ...
    # Copy public key from panel to node
    cp "$target_dir/docker-compose.yml" "${LANG[NODE_DIR]}/docker-compose.yml"
    sed -i "s|SECRET_KEY=\"PUBLIC KEY FROM REMNAWAVE-PANEL\"|SECRET_KEY=\"$(grep 'SECRET_KEY=' "$target_dir/docker-compose.yml" | cut -d'"' -f2)\"|g" "${LANG[NODE_DIR]}/docker-compose.yml"
    ...
    echo -e "${COLOR_YELLOW}=================================================${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}Панель установлена в: ${LANG[PANEL_DIR]}${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}Нода установлена в: ${LANG[NODE_DIR]}${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}=================================================${COLOR_RESET}"
...
EOL
    ...
    echo -e "${COLOR_YELLOW}=================================================${COLOR_RESET}"
    echo -e "${COLOR_GREEN}${LANG[POST_PANEL_MESSAGE]}${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}=================================================${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}${LANG[PANEL_ACCESS]}${COLOR_RESET}"
    echo -e "${COLOR_WHITE}https://${PANEL_DOMAIN}/auth/login?${cookies_random1}=${cookies_random2}${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}-------------------------------------------------${COLOR_RESET}"
    ...
    echo -e "${COLOR_YELLOW}${LANG[PASSWORD]} ${COLOR_WHITE}$SUPERADMIN_PASSWORD${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}-------------------------------------------------${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}${LANG[POST_PANEL_INSTRUCTION]}${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}-------------------------------------------------${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}${LANG[RELAUNCH_CMD]}${COLOR_RESET}"
    echo -e "${COLOR_GREEN}remnawave_reverse${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}=================================================${COLOR_RESET}"
...
add_node_to_panel() {
    ...
    echo -e ""
    echo -e "${COLOR_GREEN}[?]${COLOR_RESET} ${COLOR_YELLOW}${LANG[CONFIRM_SERVER_PANEL]}${COLOR_RESET}"
    reading "${LANG[CONFIRM_PROMPT]}" confirm
    echo
    ...
    reading "${LANG[SELFSTEAL]}" NODE_DOMAIN
    
    check_node_domain "$domain_url" "$token" "$NODE_DOMAIN"
    if [ $? -ne 0 ]; then
    ...
    # Create config profile
    echo -e "${COLOR_YELLOW}${LANG[CREATING_CONFIG_PROFILE]}${COLOR_RESET}"
    read config_profile_uuid inbound_uuid <<< $(create_config_profile "$domain_url" "$token" "StealConfig" "$NODE_DOMAIN" "$private_key")
    echo -e "${COLOR_GREEN}${LANG[CONFIG_PROFILE_CREATED]}${COLOR_RESET}"
    
    # Create node with config profile binding
    printf "${COLOR_YELLOW}${LANG[CREATE_NEW_NODE]}${COLOR_RESET}\n" "$NODE_NAME"
    create_node "$domain_url" "$token" "$config_profile_uuid" "$inbound_uuid" "172.30.0.1" "$NODE_NAME"
    
    # Create host
    echo -e "${COLOR_YELLOW}${LANG[CREATE_HOST]}${COLOR_RESET}"
    create_host "$domain_url" "$token" "$inbound_uuid" "$NODE_DOMAIN" "$config_profile_uuid"
    ...
    # Update squad
    echo -e "${COLOR_YELLOW}${LANG[UPDATING_SQUAD]}${COLOR_RESET}"
    update_squad "$domain_url" "$token" "$squad_uuid" "$inbound_uuid"
    echo -e "${COLOR_GREEN}${LANG[UPDATE_SQUAD]}${COLOR_RESET}"
...
installation_node() {
    ...
    reading "${LANG[SELFSTEAL]}" NODE_DOMAIN
    check_domain "$NODE_DOMAIN" true false
    local node_check_result=$?
    if [ $node_check_result -eq 2 ]; then
        echo -e "${COLOR_RED}${LANG[ABORT_MESSAGE]}${COLOR_RESET}"
        exit 1
    ...
    echo -e "${COLOR_WHITE}(Вставьте содержимое сертификата и нажмите Enter дважды)${COLOR_RESET}"
    
    local certificate_content=""
    local line
    local empty_lines=0
    ...
    local cert_file="$node_dir/node_cert.pem"
